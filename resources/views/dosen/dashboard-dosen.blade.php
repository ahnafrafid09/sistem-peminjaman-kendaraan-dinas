            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <div class="alert alert-info mb-4">
                    ...
                    <ul>
                        ...
                        <li>Kendaraan tidak tersedia untuk peminjaman pada tanggal 12 Mei 2025 karena agenda servis rutin.
                        </li>
                    </ul>
                </div>
                                ...
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        @foreach ($activePeminjaman as $peminjaman)
                                            <tr>
                                                <td>{{ $peminjaman->id }}</td>
                                                <td>{{ $peminjaman->mobil->plat_nomor }}
                                                    ({{ $peminjaman->mobil->merek_mobil->nama }})
                                                </td>
                                                <td>{{ $peminjaman->tanggal_peminjaman->format('Y-m-d') }}
                                                </td>
                                                <td>{{ $peminjaman->tanggal_pengembalian->format('Y-m-d') }}
                                                </td>
                                                <td>
                                                    <span
                                                        class="badge bg-{{ $peminjaman->status == 'Disetujui' ? 'success' : ($peminjaman->status == 'Ditolak' ? 'danger' : 'warning') }}">
                                                        {{ $peminjaman->status }}
                                                    </span>
                                                </td>
                                                <td>
                                                    <a href="{{ route('dosen.peminjaman.show', $peminjaman->id) }}"
                                                        class="btn btn-sm btn-info me-1">Detail</a>
                                                    @if ($peminjaman->status == 'Disetujui' && is_null($peminjaman->tanggal_pengembalian))
                                                        <a href="{{ route('dosen.pengembalian.create', $peminjaman->id) }}"
                                                            class="btn btn-sm btn-success">Ajukan Pengembalian</a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
