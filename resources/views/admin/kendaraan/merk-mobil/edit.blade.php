...
@section('content')
    <div class="container p-0 mt-5 shadow border rounded border-light border-1">
        ...
        <form action="{{ route('admin.kendaraan.merk-mobil.update', $merkMobil->id) }}" method="POST"
            enctype="multipart/form-data" class="p-4">
            @csrf
            @method('PUT')
            <div class="">
                <label for="nama" class="form-label">Merk Mobil</label>
                <input type="text" name="nama" id="nama" class="form-control" value="{{ $merkMobil->nama }}"
                    required>
            </div>
        ...
        <div class="modal fade" id="modalHapus" tabindex="-1" aria-labelledby="modalDelete" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form method="POST" action="{{ route('admin.kendaraan.merk-mobil.destroy', $merkMobil->id) }}">
                        @csrf
                        @method('DELETE')
